...
async function detectSearchNeed(messages, currentDilemma, currentOptions) {
  ...
  const isAbstract = currentOptions?.some((opt) =>
    /^(option|choice|alternative|job|plan|path)\s*[a-z0-9]?$/i.test(opt)
  );
...
export async function POST(request) {
  try {
    const { messages, currentOptions, currentCriteria, currentDilemma, currentPhase, userContext } =
      await request.json();
    ...
    try {
      if (await detectSearchNeed(messages, currentDilemma, currentOptions)) {
        ...
        const optionsContext =
          currentOptions?.length > 0 ? `Options: ${currentOptions.join(', ')}.` : '';
    ...
    const phaseInstructions =
      currentPhase === 'CRITERIA' ? CRITERIA_PHASE_INSTRUCTIONS : OPTIONS_PHASE_INSTRUCTIONS;
    ...
    const systemPrompt = CHAT_SYSTEM_PROMPT.replace('{dilemma}', currentDilemma || 'None yet')
    ...
    if (suggestedCriteria.length === 0) {
      ...
      const extractedFromText = criteriaMatches.map((m) =>
        m.replace(/\[\[Criterion:|\]\]/g, '').trim()
      );
      if (responseText.toLowerCase().includes('added') && extractedFromText.length > 0) {
