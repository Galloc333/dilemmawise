...
export async function POST(request) {
  try {
    const { winner, ranking, weights, scores, hasTie, tiedWinners, userContext, dilemma, options } =
      await request.json();

    const winnerDisplay = hasTie ? tiedWinners.map((w) => w.option).join(' and ') : winner.option;
    ...
    const dataPrompt = DATA_ANALYSIS_PROMPT.replace('{winner}', winnerDisplay)
    ...
    if (hasUserContext) {
      ...
      const personalPrompt = PERSONAL_ANALYSIS_PROMPT.replace(
        '{dilemma}',
        dilemma || 'Making a decision'
      )
