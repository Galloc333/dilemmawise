...
function generateFallbackQuestions(criteria, existingContext = {}) {
  const criteriaQuestionMap = {
    price: {
      ...
      reason: 'To evaluate value for money.',
    },
    cost: {
      field: 'budget',
      question: 'What is your budget constraint?',
      reason: 'To compare costs within your range.',
    },
    budget: {
      field: 'budget',
      question: 'What is your maximum budget?',
      reason: 'To filter options by affordability.',
    },
    quality: {
      field: 'quality_priority',
      question: 'What aspects of quality matter most to you?',
      reason: 'To focus on relevant quality factors.',
    },
    performance: {
      field: 'use_case',
      question: 'What will you primarily use this for?',
      reason: 'To evaluate performance for your needs.',
    },
    career: {
      field: 'career_goals',
      question: 'What are your career goals or priorities?',
      reason: 'To assess career fit.',
    },
    job: {
      field: 'work_preferences',
      question: 'What aspects of work are most important to you?',
      reason: 'To match job characteristics.',
    },
    location: {
      field: 'location_needs',
      question: 'What location factors matter to you (commute, neighborhood, etc.)?',
      reason: 'To evaluate location fit.',
    },
    commute: {
      field: 'commute_preference',
      question: 'How do you typically commute and how far is acceptable?',
      reason: 'To compare commute options.',
    },
    lifestyle: {
      field: 'lifestyle_priorities',
      question: 'What lifestyle factors are important to you?',
      reason: 'To match lifestyle preferences.',
    },
  };
...
function generateSearchQuery(question, options, criteria, context = {}) {
  ...
  if (
    critLower.includes('family') ||
    critLower.includes('proximity') ||
    critLower.includes('distance') ||
    critLower.includes('travel')
  ) {
    const familyLoc = context.family_location || context.home_location;
    if (familyLoc) specializedContext = `from ${familyLoc}`;
  } else if (
    critLower.includes('salary') ||
    critLower.includes('career') ||
    critLower.includes('job') ||
    critLower.includes('professional')
  ) {
    if (context.current_position) specializedContext = `for a ${context.current_position}`;
  } else if (
    critLower.includes('budget') ||
    critLower.includes('price') ||
    critLower.includes('cost')
  ) {
...
async function enrichQuestionsWithFacts(
  questions,
  options,
  criteria,
  context = {},
  ...
  return await Promise.all(
    questions.map(async (q) => {
      try {
        ...
        if (!searchQuery) {
          console.warn(
            '[enrichQuestionsWithFacts] No search query generated for question:',
            q.text
          );
          return q;
        }
        ...
        console.log(
          '[enrichQuestionsWithFacts] Found',
          results.length,
          'results for:',
          searchQuery
        );
...
- Keep concise (under 500 chars per field)`;
        ...
        try {
          ...
          if (data && !data.bullet_points && !data.takeaway && !data.charts) {
            console.warn(
              '[enrichQuestionsWithFacts] LLM returned empty/invalid data structure:',
              data
            );
          ...
          console.warn(
            '[enrichQuestionsWithFacts] JSON parse failed, using fallback:',
            parseError.message
          );
          data = null;
        }
...
Criteria: ${JSON.stringify(criteria)}${existingContextStr}`;
    ...
    if (mode === 'generate_questions') {
      ...
      try {
        ...
        console.error(
          '[generate_questions] Raw response:',
